...
import { M1_TUTORIAL_STEPS, type TutorialPad } from "@/lib/game/m1/tutorialSteps";
...
export function M1TutorialOverlay({ shellRoot, getDemoApi, onStepChange, onComplete }: M1TutorialOverlayProps) {
  ...
  const step = M1_TUTORIAL_STEPS[stepIx];
  const isDemo = !!step?.demo;
  const requiresInteraction = !!step?.interaction;

  // Add/remove interactive class on tutorial element
  useEffect(() => {
    const tutEl = tutorialRef.current;
    if (!tutEl) return;
    if (requiresInteraction && !interactionComplete) {
      tutEl.classList.add("m1-tut-interactive");
    } else {
      tutEl.classList.remove("m1-tut-interactive");
    }
    ...
    (options?: { scroll?: boolean }) => {
      ...
      if (!currentStep || currentStep.demo) {
        ...
        if (full && currentStep?.demo) {
          // For interactive demo steps, don't block clicks
  ...
  useEffect(() => {
    ...
    const cancelMeasure = scheduleAfterLayout(() => {
      ...
      const currentStep = M1_TUTORIAL_STEPS[stepIx];
  ...
  // Re-measure after brief→tutorial fade so fixed spotlight matches shell layout.
  useEffect(() => {
    if (!shellRoot) return;
  ...
  // Listen for user interactions in demo steps
  useEffect(() => {
    ...
    const handleInteraction = (e: Event) => {
      ...
      if (currentStep.interaction === "click-lead") {
        if (target.closest('.lead[data-lead="compute"]')) {
          setInteractionComplete(true);
        }
      } else if (currentStep.interaction === "click-anomaly") {
        if (target.closest('[data-anomaly="true"]')) {
          getDemoApi?.()?.showAnomaly();
          setInteractionComplete(true);
        }
      } else if (currentStep.interaction === "click-verify") {
        if (target.closest('.m1-tut-verify-btn') || target.closest('.m1-tut-anomaly-panel button')) {
          setInteractionComplete(true);
        }
      }
    };
  ...
  // Auto-advance when interaction is complete
  useEffect(() => {
  ...
  const handleNext = useCallback(() => {
    ...
    const next = stepIx + 1;
    if (next >= M1_TUTORIAL_STEPS.length) {
      finish();
      return;
    }
    setStepIx(next);
  ...
  const isLastStep = stepIx >= M1_TUTORIAL_STEPS.length - 1;
  ...
  return (
    <div
      ...
      role="dialog"
      aria-modal="true"
      aria-labelledby="m1-tut-title"
    >
      <div ref={dimRef} id="m1-tut-dim" className="m1-tut-dim" aria-hidden />
      <div ref={fullDimRef} id="m1-tut-full" className="m1-tut-full" aria-hidden />
      <div ref={highlightRef} id="m1-tut-highlight" className="m1-tut-highlight" aria-hidden />

      <div
        id="m1-tut-footer"
        ref={footerRef}
        className={`${footerPos ? "m1-tut-footer--placed" : ""}${dragging ? " m1-tut-footer--dragging" : ""}`}
        style={
          footerPos
      ...
      >
        ...
        >
          <span className="m1-tut-drag-grip" aria-hidden>
            <i className="fas fa-grip-lines" />
          </span>
          <div className="m1-tut-drag-handle-main">
            <span className="m1-tut-kicker">Tutorial</span>
            <div id="m1-tut-meta" className="m1-tut-meta">
              <span id="m1-tut-counter">
                Step {stepIx + 1} / {M1_TUTORIAL_STEPS.length}
              </span>
              <span id="m1-tut-phase">
                {step?.phase === "demo" ? "HANDS-ON" : "TOUR"}
              </span>
            </div>
            <h3 id="m1-tut-title">{step?.title}</h3>
          </div>
        </div>
        <div className="m1-tut-inner">
          <div id="m1-tut-body" dangerouslySetInnerHTML={{ __html: step?.html ?? "" }} />
          {interactionComplete && requiresInteraction && (
            <div className="m1-tut-success">
